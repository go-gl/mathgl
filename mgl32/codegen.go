...
// See the invocation in mgl32/util.go.
...
func main() {
	flag.Usage = func() {
		fmt.Println("Usage: codegen -template file.tpl -output file.go")
		fmt.Println("Usage: codegen -mgl64 [-dir ../mgl64]")
		flag.PrintDefaults()
	}

	tplPath := flag.String("template", "file.tpl", "template path")
	...
	if err = tpl.ExecuteTemplate(oFile, tplName, context); err != nil {
		panic(err)
	}
	oFile.Close()
