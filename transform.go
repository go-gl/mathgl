...
func Rotate3DY(angle float64) Matrix {
	sin, cos := math.Sin(angle), math.Cos(angle)
	return *unsafeMatrixFromSlice(ScalarSlice([]interface{}{cos, 0, sin,
		0, 1, 0,
...
func Rotate3DZ(angle float64) Matrix {
	sin, cos := math.Sin(angle), math.Cos(angle)
	return *unsafeMatrixFromSlice(ScalarSlice([]interface{}{cos, -sin, 0,
		-sin, cos, 0,
...
func Transform2D(Tx, Ty float64) Matrix {
	return *unsafeMatrixFromSlice(ScalarSlice([]interface{}{1, 0, Tx,
		0, 1, Ty,
		0, 0, 1}, FLOAT64), FLOAT64, 3, 3)
}
...
func Transform3D(Tx, Ty, Tz float64) Matrix {
	return *unsafeMatrixFromSlice(ScalarSlice([]interface{}{1, 0, 0, Tx,
		0, 1, 0, Ty,
		0, 0, 1, Tz,
...
func HomogRotate3DX(angle float64) Matrix {
	sin, cos := math.Sin(angle), math.Cos(angle)
	return *unsafeMatrixFromSlice(ScalarSlice([]interface{}{1, 0, 0, 0,
		0, cos, -sin, 0,
		0, sin, cos, 0,
		0, 0, 0, 1}, FLOAT64), FLOAT64, 4, 4)
}
...
func HomogRotate3DY(angle float64) Matrix {
	sin, cos := math.Sin(angle), math.Cos(angle)
	return *unsafeMatrixFromSlice(ScalarSlice([]interface{}{cos, 0, sin, 0,
		0, 1, 0, 0,
		-sin, 0, cos, 0,
		0, 0, 0, 1}, FLOAT64), FLOAT64, 4, 4)
}
